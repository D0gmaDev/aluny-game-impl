...
public class UnbanCommand extends Command {
    ...
    public void defaultContext(GamePlayer player, String targetName, String[] args) {
        ...
        serviceManager.getPlayerAccountService().getPlayerAccountByName(targetName)
                ...
                .ifPresentOrElse(playerAccount -> playerSanctionAPI.getPlayerSanctions(playerAccount, 30, 0)/*playerAccount.getCurrentSanctions()*/.stream()
                        ...
                        .ifPresentOrElse(sanction -> {
                            ...
                            SimpleItem validationItem = new SimpleItem(new ItemBuilder(Material.LIME_CANDLE).setDisplayName("§aValider l'annulation"), click -> {
                                click.getPlayer().closeInventory();

                                playerSanctionAPI.cancelSanction(sanction.getId()).filter(DetailedPlayerSanction::isCanceled).ifPresentOrElse(
                                        updatedSanction -> player.getMessageHandler().sendComponentMessage("moderation_sanction_canceled", Placeholder.unparsed("id", String.valueOf(updatedSanction.getId())), Placeholder.unparsed("name", playerAccount.getName())),
                            ...
                            SimpleItem infoItem = new SimpleItem(new ItemBuilder(Material.PAPER).setDisplayName(
                                    new Translation("sanction_unban_player", Placeholder.unparsed("id", String.valueOf(sanction.getId())), Placeholder.unparsed("name", playerAccount.getName()))).addLoreLines(
                        ...
                        }, () -> player.getMessageHandler().sendComponentMessage("moderation_sanction_not_found", Placeholder.unparsed("name", playerAccount.getName()))), () -> player.getMessageHandler().sendMessage("command_validation_player_not_found", targetName));
    }
